...
export const ProductVariationSaleCell = ({ 
  productId, 
  currentSaleId, 
  sales, 
  onSaleChange,
  ...
  const handleSaleChange = async (newSaleId: string) => {
    ...
    try {
      ...
      const response = await axios.patch(`/api/variants/${variantId}`, updateData)

      if (response.status === 200) {
        onSaleChange?.(newSaleId === "none" ? null : newSaleId)
        toast.success("Product Variant Sale updated successfully")
        router.refresh()
      } else {
...
interface ProductVariationDiscountCellProps {
  productId: string
    variantId: string
...
export const ProductVariationDiscountCell = ({ 
  productId, 
  currentDiscountId, 
  discounts, 
  onDiscountChange,
    variantId,
  initialDiscountName 
}: ProductVariationDiscountCellProps) => {
  const [isLoading, setIsLoading] = useState(false)
  const router = useRouter()
  ...
  const handleDiscountChange = async (newDiscountId: string) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        toast.success("Product vairation Discount updated successfully")
        router.refresh()
      } else {
        throw new Error(`Unexpected response status: ${response.status}`)
      }
...
type Collection = {
  ...
  export const ProductVariationCollectionCell = ({ 
    productId, 
    currentCollectionId, 
    collections, 
    onCollectionChange,
    ...
    const handleCollectionChange = async (newCollectionId: string) => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          toast.success("Product Variation Collection updated successfully")
          router.refresh()
        } else {
          throw new Error(`Unexpected response status: ${response.status}`)
        }
      } catch (error) {
        toast.error("Failed to update product variation discount")
        if (axios.isAxiosError(error)) {
          console.error('Axios error:', {
            message: error.message,
            response: error.response?.data,
